...
export const InvestView = ({ navigate }) => {
  ...
  const handleBuy = (type, min, max, inputValue, setInputFn) => {
    const val = parseFloat(inputValue);
    if (!val || val < min || val > max) {
      alert(`Valor deve ser entre $${min} e $${max}`);
      return;
    }
    const success = addPlan(type, val);
    if (success) {
        setInputFn('');
    }
  };
      ...
      <div className="grid gap-6">
        ...
        <Card className="border-gray-700 relative group">
          ...
          <ul className="text-xs text-gray-300 space-y-2 mb-4">
            ...
            <li className="flex justify-between"><span>{t('invest.minmax')}</span> <span className="text-white">$20 - $1,999</span></li>
          </ul>
          <input 
            type="number" 
          ...
          />
          <Button onClick={() => handleBuy('standard', 20, 1999, stdAmount, setStdAmount)} className="w-full text-sm">
